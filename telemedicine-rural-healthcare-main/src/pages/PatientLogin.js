  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/patients/login`, {
        ...
        body: JSON.stringify({ mobile: mobileNumber.toString().trim() }),
      });
      ...
      if (!window.navigator.onLine) {
        alert('Internet connection nahi hai. Kripya check karein.');
      } else {
        alert('Server se connect nahi ho pa raha. Backend chal raha hai? Port 5000 check karein.');
      }
    }
  };
