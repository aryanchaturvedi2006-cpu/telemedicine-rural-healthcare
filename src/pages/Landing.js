...
const Landing = () => {
  ...
  useEffect(() => {
    const shouldOpen = localStorage.getItem('openAarogya');
    if (shouldOpen === 'true') {
      localStorage.removeItem('openAarogya');
      setIsAarogyaModalOpen(true);
      setTimeout(() => {
        const el = document.getElementById('aarogya-section');
